...
由于是考试，没有太多时间让我去慢慢证明这两道题的设定是等价的，于是我就先把代码写出来提交上去试试，结果发现不等价，当时没有进一步的思路，于是放下了这道题，去做下一道题。(错误的代码就不太贴了)

#### 思路2：贪心[O(n), AC]

这是在考完后和室友讨论时室友提出的思路，把一列先当成一个整体，则列与列之间需要传递的金币数是可以直接算出来的，而每一列的两行之间也只需相互弥补即可。实现的过程中需要注意的是ans可能会大于$INT\_MAX$，因此ans也需要使用long long储存。
